/* N.O.V.A Live HUD — drives Claude Code, streams its telemetry.
...
async function transmit(message){
  ...
  running = false;
  clearInterval(tick);
  await speakDone;                 // don't return until N.O.V.A has finished talking
}
...
/* ══════════ continuous voice conversation ══════════
   Click VOICE once. From then on: listen → you stop → transcribe → N.O.V.A
   ...
   The mic is deliberately DEAF while N.O.V.A is thinking or speaking (`suppress`)
   — otherwise it transcribes its own voice through the speakers and talks to
   itself forever. That's why re-arming happens only after it finishes. */
...
// ── voice out ──
let muted = false, player = null, ttsDown = false;

/* Fallback voice: the browser's own speech synthesis. Free, unlimited, always
   available. Used whenever Kokoro can't deliver — not installed, model
   error — so N.O.V.A is never silently mute. */
function browserSpeak(text){
  return new Promise(resolve => {
    ...
    u.voice = v.find(x => /daniel|arthur|george|british/i.test(x.name))
           || v.find(x => x.lang === 'en-GB') || v[0];
    u.rate = 1.05; u.pitch = 0.95;
    document.body.classList.add('speaking');
    const done = () => { document.body.classList.remove('speaking'); resolve(); };
    u.onend = done; u.onerror = done;
    speechSynthesis.speak(u);
    // Safety net: some browsers never fire onend for long strings.
    setTimeout(() => { if (speechSynthesis.speaking) return; done(); },
               Math.min(30000, 900 + text.length * 70));
  });
}

/* Fallback listening: the browser's own speech recognition. Used whenever
   ElevenLabs STT can't deliver — no key, quota, network blip — so N.O.V.A
   still hears you. Chrome-based browsers only; others resolve empty. */
function browserListen(timeoutMs = 8000){
  return new Promise(resolve => {
...
async function speak(text){
  ...
  // No length cap here — Kokoro is local and free, unlike the ElevenLabs setup
  // this limit was originally written for. voice.py still caps at 8000 chars
  // as a sanity ceiling against a truly runaway response.
  const spoken = text;
  ...
  if (RT.tts && !ttsDown){
    try {
      ...
      if (!r.ok){
        // read the reason instead of throwing it away
      ...
      await new Promise((res, rej) => {
        ...
        player.onerror = () => { URL.revokeObjectURL(url); rej(new Error('playback failed')); };
        player.play().catch(rej);                     // autoplay block lands here
      });
      document.body.classList.remove('speaking');
      return;
    } catch (e){
      const why = String(e.message || e);
      // Local TTS failures (not installed, model error) are persistent, not
      // transient like a quota 429 — no point retrying Kokoro every turn.
      ttsDown = true;
      sys('voice', 'unavailable', 'warn');
      $('#p-voice').textContent = 'Voice: Kokoro unavailable — using browser voice';
      log('error','VOICE', `Kokoro failed (${why.slice(0,90)}) — switching to the browser voice.`);
      document.body.classList.remove('speaking');
    }
  }

  await browserSpeak(spoken);                         // always say something
}

// ── start / stop the whole conversation ──
async function micToggle(){
  if (convo){ stopConvo(); return; }
  if (!RT.stt && !(window.SpeechRecognition || window.webkitSpeechRecognition)){
    log('error','VOICE','no ElevenLabs key and no browser speech recognition available');
    return;
  }
  if (!RT.stt) log('voice','VOICE','no ElevenLabs key — using the browser to listen');
  try {
...
async function ship(){
  ...
  try {
    const r = await fetch('/api/listen', {method:'POST',
      ...
      .then(r=>r.json())
      .catch(e => ({error: String(e.message || e), text: ''}));   // network-level failure
    let text = (r.text||'').trim();
    const ms = Math.round(performance.now() - t0);
    if (r.error && !text){
      log('error','VOICE',`ElevenLabs STT failed (${String(r.error).slice(0,80)}) — trying browser recognition`);
      text = (await browserListen()).trim();
      if (text) log('voice','VOICE',`transcribed by browser: "${text}"`);
    } else if (text){
...
const TIPS = {
  '/new':'Fresh thread — clears the conversation and starts N.O.V.A clean.',
  '/goal':'Say your standing objective, e.g. “grow the newsletter to 5k”.',
  '/profile':'Tell N.O.V.A about you — it saves it for every future run.',
  ...
  '/kanban':'Say a task to queue it, or transmit alone to hear the queue.',
  '/tools':'Transmit alone to hear which tools and connectors are live.',
  '/commands':'Transmit alone for the full command list.'
};
let defaultTip = '';

// ── connected-tools chips (filled from the first run's STATUS event) ──
function setChips(list){
  const box = $('#chips');
  if (!list || !list.length){
    box.innerHTML = '<span class="chip muted">no connectors loaded</span>';
    return;
  }
  box.innerHTML = list.map(n =>
    `<span class="chip on">${esc(String(n).replace(/^claude\.ai /,'').replace(/^plugin:/,''))}</span>`
  ).join('');
}
...
$('#cmds2').onclick = () => transmit('/commands');
$('#clear').onclick = () => {
  $('#input').value = ''; disarm(); $('#tip').textContent = defaultTip;
  $('#response').innerHTML = '<span class="ph">Standing by. Transmit an instruction.</span>';
  setTag('', 'IDLE'); setState('', 'STANDBY', 'awaiting uplink');
};

// ── mission controls ──
document.querySelector('.mctl').addEventListener('click', e => {
  const b = e.target.closest('.mbtn'); if (!b) return;
  switch (b.dataset.mc){
    case 'status':
      log('status','STATUS', `speak=${RT.tts?'kokoro':'off'} listen=${RT.stt?'elevenlabs':'browser'} · `
        + `session=${$('#sy-session').textContent} · uplink=${now()}`);
      break;
    case 'tools':  transmit('/tools'); break;
    case 'queue':  transmit('/kanban'); break;
    case 'wipe':   $('#log').innerHTML = ''; log('note','LOG','cleared'); break;
  }
});
...
(async () => {
  ...
  try {
    ...
    RT.tts = s.tts==='kokoro'; RT.stt = s.stt==='elevenlabs';
    const browserSTT = !!(window.SpeechRecognition || window.webkitSpeechRecognition);

    const port = location.port || '80';
    sys('gw', `online · :${port}`, 'ok');
    sys('brain', 'N.O.V.A Core', 'ok');
    sys('voice', RT.tts ? 'Kokoro' : 'off', RT.tts ? '' : 'warn');
    sys('profile', 'default');
    sys('session', s.session ? s.session.slice(0,8) : 'none');
    sys('clock', now());

    // header pills
    $('#p-gw').textContent = `Gateway: online ${location.origin}`;
    const speakLine = RT.tts ? 'Kokoro' : 'unavailable';
    const listenLine = RT.stt ? 'ElevenLabs' : (browserSTT ? 'browser' : 'unavailable');
    $('#p-voice').textContent = `Voice: speak ${speakLine} / listen ${listenLine}`;
    if (!RT.tts && !RT.stt && !browserSTT) $('#p-voice').previousElementSibling?.classList.replace('am','bad');

    log('status', 'BOOT',
        `gateway online · core ready · permission=${s.permission}`);
    log('voice', 'VOICE', `channel ready — speak: ${speakLine} · listen: ${listenLine}`);
    setState('', 'STANDBY', 'awaiting uplink');
  } catch(e){
    sys('gw','offline','warn');
    $('#p-gw').textContent = 'Gateway: offline';
    $('#p-gw-dot').classList.replace('ok','bad');
