...
cd jarvis-os
cp .env.example .env                        # add your ElevenLabs key (listening)
pip install -r requirements-kokoro.txt      # local voice (speaking) — first run
                                             # downloads the ~300MB Kokoro model
./start.sh
```
...
| **Python 3** | stdlib only for everything except voice — no `npm`, no build step |
| **Kokoro** | `pip install -r requirements-kokoro.txt` — local text-to-speech, no key, no quota |
| **ElevenLabs** | free account, for speech-to-text (listening) only. Without it, the browser's own speech recognition takes over instead. |
...
Speaking and listening are independent and fail separately: Kokoro (speaking)
runs locally and needs no key, so it's never gated by a subscription; if it
isn't installed, the browser's own speech synthesis says the words instead.
ElevenLabs (listening) needs a key; without one, the browser's own speech
recognition listens instead (Chrome-based browsers only).
...
├── voice.py      Kokoro text-to-speech (local) + ElevenLabs speech-to-text
