...
Claude Code subscription and every MCP connector / skill you've configured.
Speaking (Kokoro) runs locally, no key needed. The ElevenLabs key (in .env)
is only for listening, and only ever lives server-side.
"""
import json
import mimetypes
import os
    ...
    def do_GET(self):
        ...
        if p == "/api/status":
            return self._json(dict(
                ...
                stt="elevenlabs" if voice.stt_available() else "none",
                tts="kokoro" if voice.tts_available() else "none",
                session=SESSION["id"]))
        if p == "/api/jobs":
    ...
    def do_POST(self):
        ...
        if p == "/api/speak":
            try:
                ...
                return self._bytes(voice.speak(text), "audio/wav")
...
def main():
    ...
    tts = (f"Kokoro · {voice.KOKORO_VOICE}" if voice.tts_available()
           else "none (pip install kokoro soundfile)")
    stt = ("ElevenLabs" if voice.stt_available()
           else "none (add ELEVENLABS_API_KEY, or the browser will listen instead)")
